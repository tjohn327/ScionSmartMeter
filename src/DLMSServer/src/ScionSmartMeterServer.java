...
public class ScionSmartMeterServer {
    ...
    public ScionSmartMeterServer() throws SerialPortException{
    ...
    public static void main(String[] args) throws IOException {
        ...
        if(args.length == 0){
            System.out.println("Client address needed as a command line argument");
            System.exit(1);
        }
        byte[] AUTHENTICATION_KEY;
        
        String clientAddr = args[0];
        LogicalDevice logicalDevice;
        String serverAddr;
        
        DlmsServer.ScionServerBuilder serverBuilder = null;
        ScionSmartMeterServer s = null;
        try{
            s = new ScionSmartMeterServer();
            int port = 13555;
            ReferencingMethod refMethod = ReferencingMethod.LOGICAL;
            String manufacturerId = "SCI";
            long deviceId = 12;
            serverAddr = Javaappnet.GetLocalAddress(port);
            AUTHENTICATION_KEY = parseHexBinary(Javaappnet.GetDRKey(serverAddr,clientAddr));

            SecuritySuite AUTHENTICATION_C = SecuritySuite.builder()
            ...
            logicalDevice = new LogicalDevice(1, "sci", manufacturerId, deviceId);
            ...
            serverBuilder = DlmsServer.scionServerBuilder(port)
                    .setReferencingMethod(refMethod)
                    .setInactivityTimeout(2000)
