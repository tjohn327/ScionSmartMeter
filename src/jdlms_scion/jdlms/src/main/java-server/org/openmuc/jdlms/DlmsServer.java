...
import org.openmuc.jdlms.internal.transportlayer.server.ServerScionLayer;
...
public class DlmsServer implements AutoCloseable {
     ...
     *
     * @throws IOException if an exception occurs during the start of the server.
     */
     ...
     *
     * @throws IOException if an exception occurs, while releasing the resources.
     */
     ...
     *
     * @param port the TCP port the server starts listening on.
    ...
    public static ScionServerBuilder scionServerBuilder(int port) {
        return new ScionServerBuilder(port);
    }
    ...
    public static class TcpServerBuilder extends ServerBuilder<TcpServerBuilder> {
         ...
         *
         * @param sessionLayerFactory the session layer factory.
    ...
    public static class ScionServerSettingsImpl extends ServerSettingsImpl implements ScionServerSettings {

        private final int port;

        public ScionServerSettingsImpl(int port) {
            this.port = port;
        }

        @Override
        public int getScionPort() {
            return this.port;
        }

    }

    public static class ScionServerBuilder extends ServerBuilder<ScionServerBuilder> {

        private int port;
        private ServerSessionLayerFactory sessionLayerFactory;

        private ScionServerBuilder(int port) {
            this.port = port;
            this.sessionLayerFactory = ServerSessionLayerFactories.newWrapperSessionLayerFactory();
        }
         ...
         * @see ServerSessionLayerFactories#newHdlcSessionLayerFactory()
         */
        public ScionServerBuilder setSessionLayerFactory(ServerSessionLayerFactory sessionLayerFactory) {
            this.sessionLayerFactory = sessionLayerFactory;
            return this;
        }
         ...
         * @return the current builder instance.
         */
        public ScionServerBuilder setScionPort(int port) {
            this.port = port;
            return this;
        }
        ...
        public DlmsServer build() throws IOException {
            ...
            final ScionServerSettingsImpl settings = new ScionServerSettingsImpl(this.port);
            setPropertiesTo(settings);

            ServerScionLayer serverLayer = new ServerScionLayer(settings, dataDirectory, sessionLayerFactory);
            return newServer(serverLayer);
        }
